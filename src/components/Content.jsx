...
const Content = ({ partOfSpeech, definitions }) => {
  const { res, selectedPartOfSpeech, setSelectedPartOfSpeech, isPlaying, setIsPlaying } = useContext(Context);
        ...
        <div>
          ...
          <div className="my-3">
            <div className="flex space-x-4">
              <button
                className={`text-white h-8 px-2 rounded ${selectedPartOfSpeech === "noun" ? "bg-black" : "bg-gray-500"}`}
                onClick={() => setSelectedPartOfSpeech("noun")}
              >
                Noun
              </button>
              <button
                className={`text-white h-8 px-2 rounded ${selectedPartOfSpeech === "verb" ? "bg-black" : "bg-gray-500"}`}
